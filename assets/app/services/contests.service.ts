...
//        const data = res.json().obj;
        ...
        for(let i=0; i< data.length; i++) {
          let contest = new Contest(data[i].name, data[i].idName, data[i]._id, data[i].category, data[i].description, data[i].award, data[i].status, data[i].submitions, data[i].daysRemaining, data[i].startDate, data[i].endDate, data[i].user.firstName, data[i].user._id);
          objs.push(contest);
        };
        return objs;
      })
  ...
  getIndividualContest(id: any) {
    return this.http.get('http://localhost:3000/api/v1/konkursai/' + id)
      .map(res => {
        console.log(res.json());
        const data = res.json().obj;
        let contest = new Contest(data.name, data.idName, data._id, data.category, data.description, data.award, data.status, data.submitions, data.daysRemaining, data.startDate, data.endDate,data.user.firstName, data.user._id);
        return contest;
      })
      .catch(error => Observable.throw(error.json()));
  }
