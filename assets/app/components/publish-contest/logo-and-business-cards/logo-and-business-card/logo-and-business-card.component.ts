...
export class LogoAndBusinessCardComponent implements OnInit {
  ...
  addContest(form: any) {
    if (this.contest) {
      ...
      const contest = new Contest(form.contestName.value, null, form.contestCategory.value, form.contestDescription.value, form.contestAward.value, 'active', 10, 5, Date.now(), Date.now());
      console.log(contest);
